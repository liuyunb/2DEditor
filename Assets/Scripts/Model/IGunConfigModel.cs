...
{
    GunConfigItem GetGunConfigItemByName(string name);
}
...
public class GunConfigItem
{
    ...
    public int BulletMaxCount { get; set; }//最大子弹填充数量
...
public class GunConfigModel : AbstractModel, IGunConfigModel
{
    ...
    private Dictionary<string, GunConfigItem> _mItems = new Dictionary<string, GunConfigItem>()
    {
    ...
    public GunConfigItem GetGunConfigItemByName(string name)
    {
        if (_mItems.ContainsKey(name))
            return _mItems[name];
        return null;
    }
}
